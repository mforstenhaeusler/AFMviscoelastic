...
def contact_mode(type, time, timestep, zb_init, vb, u, q, k_m1, fo1, Q1, vdw, R, nu=0.5, z_contact=0.0, F_trigger=800.0e-9, H=2.0e-19, a=0.2e-9):
    """
    Description:
    ...
    Parameters:
    :param type:        string
                        Type of Contact Mode Simulation. ('Approach' or 'Approach-Retract')
    ...
    tip_wu = 0  # tip position on the way up if F = 0 used to calculate vdW Forces and deformation correctly

    print('Number of Iterations:', len(time) / 1e6, 'Mio')

    if type == 'Approach-Retract':  # 'Approach-Retract'
        if zb_initial > z_contact:  # case if there is indentation history
            for k in np.arange(len(time)):  # advancing in time
                if k <= len(time)/2:  # Approach
                    if Fts < F_trigger:
                    ...
                    if Fts < F_trigger:
                        # Euler Integration calculating the base position
                        zb = zb + vb * timestep  # iterates base position
                        ...
                        if TipPos < z_contact:  # contact
                            ...
                            if F[k, 0] < 0:  # loss of contact on way up, when F <= 0
                                for deri in range(len(u)):  # set all force derivatives to 0
                                    F[k, deri] = 0

                                Fts = F[k, 0]  # initialize tip-sample force variable
                                ...
                                if vdw < 0.5:  # no vdW interaction
                                    ...
                                    F_ts.append(Fts - H * R / (6 * pow(tip_wu_norm + a, 2)))  # stored tip-sample force
                            else:  # no contact
                                Fts = F[k, 0]
                            ...
                            if vdw > 0.5:  # van der Waal interaction
                                if k <= len(time) / 2:  # vdW calculation for approach
                                    F_ts.append(- H * R / (6 * (TipPos + a) ** 2))  # stored tip-sample force
                                else:  # vdW calculation for retract
                                    tip_wu_init.append(TipPos)
                                    tip_wu_norm = TipPos - tip_wu_init[0]  # Normalizing tip position when F=0 to 0
                                    F_ts.append(Fts - H * R / (6 * pow(tip_wu_norm + a, 2)))  # stored tip-sample force
                            else:  # no van der Waal interaction
                        ...
                        if k % 1000000 == 0:  # print ever 100k Iteration
                            print('Iteration:', k / 1e6, 'Mio')
                    else:
                        print('F_trigger:', F_trigger)
                        print(k)
        ...
        if zb_initial == z_contact:  # case: no indentation history, k=0, all values are 0
...
def contact_mode_limited(time, timestep, zb_init, vb, u, q, k_m1, fo1, Q1, vdw, R, nu=0.5, z_contact=0.0, F_trigger=800.0e-9, H=2.0e-19, a=0.2e-9):
    """
    Description:
    This function simulates the AFM technique - Contact Mode for an arbitrary number of characteristic
...
def contact_mode_specific(t, timestep, zb_init, vb, u, q, k_m1, fo1, Q1, vdw, R, nu=0.5, z_contact=0.0, F_trigger=800.0e-9, H=2.0e-19, a=0.2e-9):
    """
    Description:
    This function simulates the AFM technique - Contact Mode for a 3 and 5-Arm Generalized Maxwell/
    Kelvin-Voigt Model.
    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Parameters:
